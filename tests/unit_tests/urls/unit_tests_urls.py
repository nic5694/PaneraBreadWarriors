...
def test_create_url_service_success(mock_url_create):
    ...
    # Corrected from .return_text to .return_value
    # We create a mock object that acts like a Peewee model instance
    ...
    # Assert - Checking the dict returned by the service
...
def test_create_url_service_validation():
    ...
    with pytest.raises(ValueError) as exc:
        service.create_url({"user_id": "401", "shortcode": "test"})
    
    assert "original_url are required" in str(exc.value)
...
def test_resolve_shortcode_service(mock_get):
    ...
    # Simulate finding the URL in the database
...
def test_resolve_shortcode_route_redirect(client):
    """Test that the redirect route (302) works correctly for a valid shortcode."""
    with patch('app.routes.urls.url_service.resolve_shortcode') as mocked_resolve:
        mocked_resolve.return_value = "https://gatech.edu"
        
        # Testing the redirect logic at the top-level route
        response = client.get("/r/gt")
